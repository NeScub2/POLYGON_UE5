...
namespace ZeroGUI
{
	...
	void Draw_Cursor(AHud* hud, bool toogle)
	{
		if (toogle)
		{
			FVector2D cursorPos = CursorPos();
			hud->DrawLine( FVector2D{ cursorPos.X, cursorPos.Y }, FVector2D{ cursorPos.X + 35, cursorPos.Y + 10 }, 1, FLinearColor{ 0.30f, 0.30f, 0.80f, 1.0f });
			...
			while (y != 30) //20 steps
			{
				...
				hud->DrawLine( FVector2D{ cursorPos.X, cursorPos.Y }, FVector2D{ cursorPos.X + x, cursorPos.Y + y }, 1, FLinearColor{ 0.30f, 0.30f, 0.80f, 1.0f });
			}

			hud->DrawLine( FVector2D{ cursorPos.X, cursorPos.Y }, FVector2D{ cursorPos.X + 15, cursorPos.Y + 30 }, 1, FLinearColor{ 0.30f, 0.30f, 0.80f, 1.0f });
			hud->DrawLine( FVector2D{ cursorPos.X + 35, cursorPos.Y + 10 }, FVector2D{ cursorPos.X + 15, cursorPos.Y + 30 }, 1, FLinearColor{ 0.30f, 0.30f, 0.80f, 1.0f });
		}
	}
	...
	void TextLeft(GEngine* gEngine, char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		...
		hud->HDrawText(FString{ s2wc(name) }, color, pos.X, pos.Y,gEngine->MediumFont,  0.97f, false);
	}
	...
	void Draw_Line(FVector2D from, FVector2D to, int thickness, FLinearColor color)
	{
		hud->DrawLine( FVector2D{ from.X, from.Y }, FVector2D{ to.X, to.Y }, thickness, color);
	}
	void drawFilledRect(FVector2D initial_pos, float w, float h, FLinearColor color)
	{
		for (float i = 0.0f; i < h; i += 1.0f)
			hud->DrawLine( FVector2D{ initial_pos.X, initial_pos.Y + i }, FVector2D{ initial_pos.X + w, initial_pos.Y + i }, 1.0f, color);
	}
